...
void build_segment_tree(int i , int lo, int hi , vector<int> &input){
    ...
    int min_val = min(segment_tree[2*i+1].first, segment_tree[2*i+2].first);
    int count = 0;

    if(segment_tree[2*i+1].first == min_val) count += segment_tree[2*i+1].second;
    if(segment_tree[2*i+2].first == min_val) count += segment_tree[2*i+2].second;

    segment_tree[i] = {min_val, count};
}

void update(int i, int lo , int hi , int idx_to_update , int val_to_update){

    if(lo == hi){
        segment_tree[i]= {val_to_update, 1};
        return;
    }
...
pp get_minimum(int i, int lo , int hi , int &l ,int &r){
    ...
    pp left_min = get_minimum(2*i+1, lo, mid, l, r);
    pp right_min = get_minimum(2*i+2, mid+1, hi, l, r);
...
void solve(){
    ...
    // for(int i = 0; i<10; i++){
    //     cout<<i<<" - > ";
    //     cout<<segment_tree[i].first<<" "<<segment_tree[i].second<<endl;
    // }
    // cout<<endl;
